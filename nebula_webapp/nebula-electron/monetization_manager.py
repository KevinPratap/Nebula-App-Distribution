...
# Suppress insecure request warnings for restricted network fallbacks
...
class MonetizationManager:
    ...
    def sync_server_time():
        ...
        try:
            ...
            if server_date_str:
                ...
                sys.stderr.write(f"DEBUG: Time Sync - Offset: {MonetizationManager._clock_offset:.2f}s\n")
                sys.stderr.flush()
        except Exception as e:
            if "CERTIFICATE_VERIFY_FAILED" in str(e):
                try:
                    # Fallback to unverified for environments with SSL issues
                    res = requests.head(MonetizationManager.SERVER_URL, timeout=5, verify=False)
                    server_date_str = res.headers.get('Date')
                    if server_date_str:
                         server_time = parsedate_to_datetime(server_date_str).timestamp()
                         local_time = time.time()
                         MonetizationManager._clock_offset = server_time - local_time
                         sys.stderr.write(f"DEBUG: Time Sync - Offset: {MonetizationManager._clock_offset:.2f}s (Insecure Fallback)\n")
                         sys.stderr.flush()
                except:
                    sys.stderr.write(f"DEBUG: Time Sync Error: {e}\n")
                    sys.stderr.flush()
            else:
                sys.stderr.write(f"DEBUG: Time Sync Error: {e}\n")
                sys.stderr.flush()
        ...
        try:
            ...
            if "CERTIFICATE_VERIFY_FAILED" in str(e):
                try:
                    hwid = MonetizationManager.get_hwid()
                    res = requests.post(f"{MonetizationManager.SERVER_URL}/register", 
                                      json={"email": email, "password": password, "hwid": hwid}, timeout=30, verify=False)
                    try:
                        return res.status_code == 201, res.json().get("message") + " (Insecure)"
                    except:
                        return False, f"Server Error {res.status_code}: {res.text[:50]} (Insecure)"
                except: pass
            return False, str(e)
    ...
    def login(email, password):
        try:
            ...
            if "CERTIFICATE_VERIFY_FAILED" in str(e):
                try:
                    res = requests.post(f"{MonetizationManager.SERVER_URL}/login", 
                                       json={"email": email, "password": password}, timeout=30, verify=False)
                    if res.status_code == 200:
                        token = res.json().get("access_token")
                        MonetizationManager.save_session(token)
                        return True, "Success (Insecure)"
                    return False, res.json().get("message", "Login Failed")
                except: pass
            return False, str(e)
    ...
    def validate_session():
        try:
            ...
            if not token: return False, {"plan": "GUEST"}
            
            headers = {"Authorization": f"Bearer {token}"}
            res = requests.get(f"{MonetizationManager.SERVER_URL}/me", headers=headers, timeout=10)
            if res.status_code == 200:
                return True, res.json()
            return False, {"plan": "GUEST"}
        except Exception as e:
            if "CERTIFICATE_VERIFY_FAILED" in str(e):
                try:
                    token = MonetizationManager.load_session() # Re-load token for fallback
                    if not token: return False, {"plan": "GUEST"}
                    headers = {"Authorization": f"Bearer {token}"}
                    res = requests.get(f"{MonetizationManager.SERVER_URL}/me", headers=headers, timeout=10, verify=False)
                    if res.status_code == 200:
                        return True, res.json()
                except: pass
            return False, {"plan": "GUEST"}
    ...
    def consume_credit():
        ...
        try:
            ...
            if "CERTIFICATE_VERIFY_FAILED" in str(e):
                try:
                    res = requests.post(f"{MonetizationManager.SERVER_URL}/api/consume", headers=headers, timeout=10, verify=False)
                    if res.status_code == 200:
                        data = res.json()
                        return True, "Credit applied (Insecure)", data.get("credits", 0)
                except: pass
    ...
    def get_ai_config():
        ...
        try:
            token = MonetizationManager.load_session()
            if not token: 
                sys.stderr.write("DEBUG: Cloud Sync - No token found locally.\n")
                sys.stderr.flush()
                return None
            ...
            if res.status_code == 200:
                config = res.json()
                sys.stderr.write(f"DEBUG: Cloud Sync - Success. Response: {list(config.keys())}\n")
                sys.stderr.flush()
                return config
            else:
                sys.stderr.write(f"DEBUG: Cloud Sync - Failed. HTTP {res.status_code}: {res.text[:100]}\n")
                sys.stderr.flush()
                return None
        except Exception as e:
            if "CERTIFICATE_VERIFY_FAILED" in str(e):
                try:
                    ...
                    if res.status_code == 200:
                        config = res.json()
                        sys.stderr.write(f"DEBUG: Cloud Sync - Success (Insecure Fallback). Response: {list(config.keys())}\n")
                        sys.stderr.flush()
                        return config
                except: pass
            sys.stderr.write(f"DEBUG: Cloud Sync - Error: {e}\n")
            sys.stderr.flush()
            return None
        ...
        try:
            if os.path.exists(MonetizationManager.SETTINGS_FILE):
                with open(MonetizationManager.SETTINGS_FILE, "r") as f:
                    ...
                    sys.stderr.write(f"DEBUG: Checking Session - Now: {now}, Expiry: {expiry}, Active: {now < expiry}\n")
                    sys.stderr.flush()
                    if now < expiry:
                        return True
            else:
