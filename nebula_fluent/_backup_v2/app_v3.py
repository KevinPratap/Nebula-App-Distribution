...
# speech_recognition will be lazy-loaded in VoiceListener
SPEECH_AVAILABLE = True # Assume available for now, checked on first use
...
class VoiceListener:
    ...
    def get_audio_devices():
        """Get list of available audio output devices for loopback"""
        devices = []
        try:
            import soundcard as sc
            # Get speakers/output devices for loopback capture
    ...
    def __init__(self, callback, use_loopback=True, source_label="Interviewer", language="en-US", ai_instance=None):
    ...
    def _process_loop(self, continuous):
        ...
        while self.is_listening or not self.recognition_queue.empty():
            try:
                ...
                if continuous:
                    # V29: Increased patience for slow speakers (2.0s silence)
                    # Trigger if pause detected (min 1.0s audio) OR we hit the 10s fallback
                    should_process = (len(frames) >= 4 and silence_counter >= 8) or (len(frames) >= 40)
...
class DualVoiceListener:
    ...
    def __init__(self, callback, language="en-US", ai_instance=None):
        self.interviewer_listener = VoiceListener(callback, use_loopback=True, source_label="Interviewer", language=language, ai_instance=ai_instance)
        self.me_listener = VoiceListener(callback, use_loopback=False, source_label="Me", language=language, ai_instance=ai_instance)
...
# ... (omitted similar lines) ...
    ...
    def start_push_to_talk(self):
        ...
        self.voice_listener = VoiceListener(self.on_push_to_talk_result, use_loopback=True, ai_instance=self.ai)
    ...
    def on_live_assist_toggle(self):
        ...
        if self.live_assist_mode.get():
            ...
            self.dual_listener = DualVoiceListener(self.on_voice_input, language=lang_code, ai_instance=self.ai)
...
class SettingsWindow:
    ...
    def setup_general_tab(self):
        ...
        self.ghost_switch.pack(anchor="w", padx=20, pady=(5, 20))
    ...
    def setup_account_tab(self):
        ...
        # User details
        ctk.CTkLabel(header, text="Logged in as", font=ctk.CTkFont(size=10, weight="bold"), text_color="#64748B").pack(anchor="w")
...
if __name__ == "__main__":
    ...
    if auth_data_json:
        ...
        # If no auth data provided, force-launch the React Login UI
        print("No NEBULA_AUTH_DATA found. Launching React Login UI...")
        relaunch_login_ui()
